    ...
    public class HealerAI : MonsterAIProfile
    {
        public override BattleAction DecideAction(MonsterController self, BattleContext context)
        {
            ...
            if (!isSilenced)
            {
                ...
                if (dyingAllies.Count > 0)
                {
                    ...
                    if (availableHeals.Count > 0)
                    {
                        ...
                        // 액션 생성 및 반환
                        var action = new BattleAction(self.gameObject, targetObj, ActionType.Skill, self.GetTotalAgi() + 10);
            ...
            if (livingPlayers.Count > 0)
            {
                ...
                return new BattleAction(self.gameObject, target.gameObject, ActionType.Attack, self.GetTotalAgi());
            }
