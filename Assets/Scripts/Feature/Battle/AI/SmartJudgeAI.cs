    ...
    public class SmartJudgeAI : MonsterAIProfile
    {
        public override BattleAction DecideAction(MonsterController self, BattleContext context)
        {
            ...
            if (livingPlayers.Count > 0)
            {
                ...
                var action = new BattleAction(self.gameObject, target.gameObject, ActionType.Attack, self.GetTotalAgi());
            ...
            {
                var action = new BattleAction(self.gameObject, weakPlayer.gameObject, ActionType.Attack, self.GetTotalAgi() + 5);
            ...
            if (!isSilenced)
            {
                ...
                if (dyingAlly != null && (float)dyingAlly.currentHp / dyingAlly.maxHp <= 0.4f)
                {
                    ...
                    if (healSkill != null)
                    {
                        var action = new BattleAction(self.gameObject, dyingAlly.gameObject, ActionType.Skill, self.GetTotalAgi() + 10);
                ...
                if (aoeSkill != null)
                {
                    var action = new BattleAction(self.gameObject, self.gameObject, ActionType.Skill, self.GetTotalAgi());
            ...
            // 폴백 설정
            return new BattleAction(self.gameObject, self.gameObject, ActionType.Guard, self.GetTotalAgi() + 2000);
        }
